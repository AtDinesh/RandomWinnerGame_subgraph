...
export class PlayerEntered extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save PlayerEntered entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type PlayerEntered must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("PlayerEntered", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): PlayerEntered | null {
    return changetype<PlayerEntered | null>(
      store.get_in_block("PlayerEntered", id.toHexString())
    );
  }

  static load(id: Bytes): PlayerEntered | null {
    return changetype<PlayerEntered | null>(
      store.get("PlayerEntered", id.toHexString())
    );
  }
